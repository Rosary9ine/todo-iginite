...
app.put('/todos/:id', checksExistsUserAccount, (request, response) => {
  ...
  const todoMustBeUpdated = user.todos.find(todo => todo.id === id);

  if(!todoMustBeUpdated) {
    return response.status(404).json({ error: "Todo Not Found" });
  }

  todoMustBeUpdated.title = title;
  todoMustBeUpdated.deadline = new Date(deadline);

  return response.status(200).json(todoMustBeUpdated);
});
